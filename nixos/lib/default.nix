...
let
  ...
  typePassthruOrLoadCredentialPath = attrs: with types;
    (
      if hasAttr "passthru" attrs
      then addCheck types.anything (x: x == attrs.passthru || path.check x && !hasPrefix builtins.storeDir x)
      else addCheck path (p: !hasPrefix builtins.storeDir p)
    ) // {
...
in
{
  ...
  toLoadCredentialList = config: map (cred: "${cred.id}:${cred.path}") (collect isLoadCredentialOption config);
}
